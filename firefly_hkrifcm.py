# -*- coding: utf-8 -*-
"""
Created on Wed Jul  4 18:54:27 2018
...
def clustering(centroids,num_centroids):
    ...
    while cond==0:
        ...
        for i in range(num_centroids):
            distki = abs(2*(np.tanh(-((pixels - centroids[i])**2)/(2*sd**2))))
            for j in range(num_centroids):
                distkj = abs(2*(np.tanh(-((pixels - centroids[j])**2)/(2*sd**2))))
...
def db(centroids,num_centroids):
    ...
    for i in range(num_centroids):
        if (np.sum(b_mem[i]) == 0):
            distki = abs(2*(np.tanh(-(f((pixels * mem[i]) - centroids[i])**2)/(2*sd**2))))
            s_v[i] = np.sum(distki**2) / np.sum(mem[i])
        elif (np.sum(mem[i]) == 0):
            distki = abs(2*(np.tanh(-(f((pixels * b_mem[i]) - centroids[i]) ** 2) / (2*sd**2))))
            s_v[i] = np.sum((t_b_mem[i]**m)*(distki**2)) / np.sum(t_b_mem[i]**m)
        else:
            distki = abs(2 * (np.tanh(-(f((pixels * mem[i]) - centroids[i]) ** 2) / (2*sd**2))))
            distkj = abs(2 * (np.tanh(-(f((pixels * b_mem[i]) - centroids[i]) ** 2) / (2*sd**2))))
            s_v[i] = wlow * (np.sum(distki**2) / np.sum(mem[i])) + wup * (np.sum((t_b_mem[i]**m)*(distkj**2)) / np.sum(t_b_mem[i]**m))

    for i in range(num_centroids):
        for j in range(num_centroids):
            if i!=j:
                dist=abs(2*(np.tanh(-((centroids[i] - centroids[j])**2)/(2*sd**2))))
...
def ffa_clustering(centroids,numclusters,num_fireflies):
    ...
    for i in range(num_fireflies):
        ...
        for k in range(numclusters):
            distki=abs(2*(np.tanh(-((pixels - centroids[i][k])**2)/(2*sd**2))))
            tmp2=np.zeros(num_data)
            for j in range(numclusters):
                distkj=abs(2*(np.tanh(-((pixels - centroids[i][j])**2)/(2*sd**2))))
    ...
    for i in range(num_fireflies):
        add=0
        for j in range(numclusters):
            dist=abs(2*(np.tanh(-((pixels - centroids[i][j])**2)/(2*sd**2))))**2
            tmp=(mem[i][j]**m)*dist
            add=add+np.sum(tmp)
        err.append(add)
    return err

im=Image.open("C:/Users/Anmol Agrawal/Desktop/river-valley .jpg")
